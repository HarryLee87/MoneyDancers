...
import {getTotalIncomeForEachCategory} from '../services/GetAccountDataQueries';
...
export default function ChartIncome() {
  ...
  const chartData = incomeByCategory.map((category, str) => ({
    name: category.name,
            ...
            <View
              ...
              <View
                ...
                <Text style={styles.text_in_row}>
                  {category.incomePercentage}% {category.name}
                </Text>
              </View>
              <View>
                <Text style={styles.text_in_row}>
                  {category.totalIncome.toLocaleString('en-US', {
                    style: 'currency',
                    currency: 'USD',
